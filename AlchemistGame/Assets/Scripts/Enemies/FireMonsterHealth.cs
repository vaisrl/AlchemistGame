...
public class FireMonsterHealth: MonoBehaviour
{
    ...
    private void OnTriggerEnter2D(Collider2D collision)
    {
        ...
        {
            if (collision.tag.Equals("FrostBall")) 
            {
                Destroy(collision.gameObject);
                TakeDamage(GameController.singleton.damage * 2);
            }
            else if (collision.tag.Equals("FireBall")) 
            {
                Destroy(collision.gameObject);
    ...
    private void Die()
    {
        // тут аанимации и партиклы... Потом надо будет сделать корутину
        Destroy(gameObject);
    }
